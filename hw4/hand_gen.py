...
def get_eigenvalues(data):
    ...
    NEIGHBOR = 25 # pick some neighbor to compute the eigenvalues
...
y = []
ratio = 0.05
maxdim = int(245760*ratio*ratio)
for i in range(maxdim):
    dim = i + 1
    print(i)
    for N in [100,200,481]:
        layer_dims = [np.random.randint(int(maxdim*0.2), int(maxdim*0.5)), np.random.randint(int(maxdim*0.5), int(maxdim*0.7)), maxdim]
