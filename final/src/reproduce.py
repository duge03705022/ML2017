...
sj_train,sj_valid,sj_t_value,sj_v_value = train_test_split(sj_merged_train[t_predictors], sj_value, test_size=0.001)
iq_train,iq_valid,iq_t_value,iq_v_value = train_test_split(iq_merged_train[t_predictors], iq_value, test_size=0.001)
...
n_model = 1
sj_model = xgb.Booster(params)
iq_model = xgb.Booster(params)
sj_model.load_model("model/sj_18.model" )
iq_model.load_model("model/iq_18.model" )



sj_models = []
iq_models = []
sj_models.append([pickle.load(open("model/sj_18.pkl" ,'rb')),sj_model])
iq_models.append([pickle.load(open("model/iq_18.pkl" ,'rb')),iq_model])
# sj_models.append([pickle.load(open("model/%s.pkl" % sys.argv[1],'rb'))])
# iq_models.append([pickle.load(open("model/%s.pkl" % sys.argv[2],'rb'))])
...
result[['city','year','weekofyear','total_cases']].to_csv( 'result/redo.csv', index=False)
